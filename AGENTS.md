...
- Full build: `./mvnw clean verify` — runs the format checks (editorconfig + Spotless, validate
...
- `./mvnw install` installs the jar into the local repo. `./mvnw deploy` publishes it to GitHub
  ...
  (server id `github`).
- `./mvnw validate` runs only the format checks (editorconfig + Spotless).
...
- Java and general file rules (indentation, line length, line endings): `editorconfig-maven-plugin`
  (`check` goal) → fix with `./mvnw editorconfig:format`.
